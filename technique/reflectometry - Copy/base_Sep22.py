...
UPDATED: Sep 2022 for Cycle 2022_3. Renaming functions.
"""
import sys
...
def run_angle_SM(sample, angle, count_uamps=None, count_seconds=None, count_frames=None, vgaps: dict = None,
                          hgaps: dict = None, smangle=0.0, mode=None, do_auto_height=False, laser_offset_block="KEYENCE",
...
def slit_check(sample=None, theta: float = 0.7, footprint: float = None, resolution: float = None):
    """
...
def auto_height(laser_offset_block: str, fine_height_block: str, target: float = 0.0, continue_if_nan: bool = False,
    ...
    EDIT: logic added to do nothing if KEYENCE in alarm state, just prints in a note. Might need to change to alarm/warning.
...
class _Movement(object):
    ...
    def count_for_store(self, count_uamps, count_seconds, count_frames, store=True):
        """
        Count for one of uamps, seconds, frames if not None in that order
        :param count_uamps: number of uamps to count for; None count in a different way
        :param count_seconds: number of seconds to count for; None count in a different way
        :param count_frames: number of frames to count for; None count in a different way
        :param store: create a saved snapshot of run at various intervals until the end. Needs testing!
        """
        ...
        if count_uamps is not None:
            ...
            if not self.dry_run:
                g.begin()
                if store:
                    current_counts = eval('g.get_uamps()')
                    while current_counts < (count_uamps - 2): #decide tolerance
                        g.snapshot_crpt("U:\\CurrentRun\current_run.nxs") #decide where to store
                        g.waitfor_time(seconds=120) # decide wait
                        current_counts = eval('g.get_uamps()')
                else:
                    g.waitfor_uamps(count_uamps)
                g.end()
            ...
            if not self.dry_run:
                g.begin()
                if store:
                    current_counts = eval('g.get_time_since_begin(False)')
                    while current_counts < (count_uamps - 240): #decide tolerance
                        g.snapshot_crpt("U:\\CurrentRun\current_run.nxs") #decide where to store
                        g.waitfor_time(seconds=120) # decide wait
                        current_counts = eval('g.get_time_since_begin(False)')
                else:
            ...
            if not self.dry_run:
                ...
                g.begin()
                if store:
                    current_counts = eval('g.get_frames()')
                    while current_counts < (final_frame - 2400): #decide tolerance
                        g.snapshot_crpt("U:\\CurrentRun\current_run.nxs") #decide where to store
                        g.waitfor_time(seconds=120) # decide wait
                        current_counts = eval('g.get_frames()')
                else:
