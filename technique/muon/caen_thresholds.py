...
class CaenThresholdSetter:
    ...
    def find_cards(self) -> dict[int, int]:
        ...
        Returns:
            dict[int, int]: A dictionary of the number of cards on each connected crate
                keys are crate number, values are numbers of crates

        Raises:
            Exception: Any errors raised by genie_python are propogated through, after checking
                all crates
        """
        num_cards = {}
        errors = []
    ...
    def set_thresholds(self,
        ...
        """
        Sets the thresholds and "enabled" status of every channel on every card in every crate.
        Can be supplied a single number or a file.

        Args:
            threshold_scale (optional): The value by which absolute threshold is scaled, if it
                is used. Defaults to the value provided upon init
            dsc_width (optional): The discriminator widths to be set. Defaults to the value
                provided upon init
            absolute_threshold (optional): The value to send uniformly to all channels, if no
                table is supplied. Defaults to thevalue provided upon init. NOTE: The  value
                sent to the CAEN will be rounded down to an integer AFTER multiplying this by
                threshold_scale.
            re_enable: Whether to ignore the table of which channels to enable and simply enable
                them all.
            threshold_table: The  path to a csv file containing the values to be set to each
                channel. If this argument is provided, the table will take priority over absolute
                thresholds
        ...
        Raises:
            ValueError: If any provided table has the incorrect number of cards in a crate, or
                channels in a card.
            ValueError: If an invalid path to a csv file of voltage thresholds is provided
            ValueError: If an IOC specified does not exist in reality
        """
            ...
            if os.path.isfile(threshold_table) and os.path.splitext(threshold_table)[1] == ".csv":
                ...
                with (open(threshold_table) as table_file):
                    ...
                    for row in table_reader:
                        if not row:
                            # If there's an empty row, we interpret it as a delimiter between
                            # crates, so we first check that the current crate has the number
                            # of cards (and throw an error if not), then move on to the next crate
                            if len(voltage_threshold_table[crate_number])\
                                    != num_cards[crate_number]:
                                raise ValueError(
                                    f"Incorrect number of cards in crate, "
                            ...
                            raise ValueError(
                                f"Incorrect number of channels in cards in {threshold_table}, "
                ...
                raise ValueError(
                    f"Incompatible file path {threshold_table}, "
                    f"please provide a valid path to a .csv file.")
        elif absolute_threshold:
            # Setting all channels to the same value
            for crate in voltage_threshold_table:
                voltage_threshold_table[crate] = ([[absolute_threshold * threshold_scale] * 16]
                                                  * num_cards[crate])
        ...
        if re_enable:
            ...
            with open(self.enable_table_path) as enable_table_file:
                ...
                for row in enable_table_reader:
                    if not row:
                        # If there's an empty row, we interpret it as a delimiter between crates,
                        # so we first check that the current crate has the number of cards (and
                        # throw an error if not), then move on to the next crate
        ...
        for crate in voltage_threshold_table:
            ...
            for card in range(num_cards[crate]):
                ...
                for channel in range(16):
                    # for each channel, set the threshold and enable based on the tables
                    # constructed
    ...
    def tweak_thresholds(self, value: int) -> None:
        ...
        Args:
            value: The amount you wish to move every channel by
        ...
        for crate in range(num_crates):
            ...
            for card in range(num_cards[crate]):
                ...
                for channel in range(16):
                    ...
                    g.set_pv(
                        f"{crate_pref}:C{card}:CH{channel}:THOLD:SP",
                        int(old_threshold + value))
        print("All cards set.")
