...
class _Movement(object):
    ...
    def update_title(self, title, subtitle, theta, smangle=None, smblock='SM', add_current_gaps=False):
        ...
        if add_current_gaps:
            ...
            try:
                ...
                vgaps = self.get_gaps(vertical=True, slitrange=['1', '2', '3'])
    ...
    def set_slit_vgaps_old(self, theta: float, constants, vgaps: dict, sample):
        """
    ...
    def setup_measurement(self, mode=None, periods=1):
        ...
        ## OFFSPEC doesn't have monitor block. Comment out for now and handle properly in new version.
        # try: 
            # g.cset("MONITOR", "IN")
        # except:
    ...
    def sample_setup(self, sample, angle, inst_constants, mode, trans_offset=0.0, smang=0.0, smblock='SM2'):
        ...
        ### Edit to remove error with SM1 and SM2 carried over from INTER. Handling will be better in new version.
...
class _Movement_copy(object):
    ...
    def osc_slit_setup(self, slit_block: str, slit_gap: float = None, slit_extent: float = None):
        ...
        Returns: block to be used as the centre point, prior centre point for resetting, min and max of movement.
        """
        # TODO: Add these into constants.py and lookup from there.
        # Modded for OFFSPEC temporarily until that is done.
        HG_defaults = {'S1HG': 30, 'S2HG': 35, 'S3HG': 40, 'S4HG': 53}
        if not slit_extent:
            try:
                slit_extent = HG_defaults[slit_block]
            except:
