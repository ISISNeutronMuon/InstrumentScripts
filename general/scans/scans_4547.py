...
class Scan(object):
    ...
    def _profile_pre_loop(self, detector):
        import warnings
        ...
        return detector, fignums, fig, axis, xs, ys, action_remainder

    def _profile_append_data(self, value, xs, ys, position):
    ...
    def _profile_write_logfile(self, logfile, xs, ys):
        logfile.write("{}\t{}\t{}\n".format(xs[-1], str(ys[-1]),
                                            str(ys[-1].err())))

    def _profile_plot(self, axis, label, xs, ys):
        axis.clear()
    ...
    def _profile_action(self, action, action_remainder, xs, ys, axis):
        if action:
            action_remainder = action(xs, ys, axis, action_remainder)
        return action_remainder

    def _profile_savefig(self, fig, save):
        fig.savefig(save)

    def _profile_next(self, x):
    ...
    def plot(self, detector=None, save=None,
        ...
        detector, fignums, fig, axis, xs, ys, action_remainder = self._profile_pre_loop(detector)
        try:
            with open(self.defaults.log_file(), "w") as logfile, \
                 detector(self, save=save, **kwargs) as detect:
                for x in self:
                    # FIXME: Handle multidimensional plots
                    (label, position) = self._profile_next(x)
                    value = detect(**just_times(kwargs))
                    value, xs, ys = self._profile_append_data(value, xs, ys, position)
                    self._profile_write_logfile(logfile, xs, ys)
                    self._profile_plot(axis, label, xs, ys)
                    action_remainder = self._profile_action(action, action_remainder, xs, ys, axis)
                    self._profile_draw()

        except KeyboardInterrupt:  # pragma: no cover
            pass
        if isinstance(save, str):
            self._profile_savefig(fig, save)
...
class SimpleScan(Scan):
    ...
    def _profile_sscan_action(self, i):
        self.action(i)

    def _profile_sscan_waitfor_move(self):
        start_time = datetime.now()
        g.waitfor_move()

    def _profile_sscan_create_dic(self):
        dic = OrderedDict()
    ...
    def __iter__(self):
        for i in self.values:
            self._profile_sscan_action(i)
            self._profile_sscan_waitfor_move()
            dic = self._profile_sscan_create_dic()
            yield dic
