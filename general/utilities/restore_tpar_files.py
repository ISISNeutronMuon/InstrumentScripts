...
def _get_y_n(question: str) -> bool:
    """
    A basic wrapper for input() that asks the user a yes or no question and returns True or False
    based on their answer, prompting them further if they don't give correct responses.
...
def tpar_check_dir() -> str | None:
    """
    A function which opening the file in which the path to the tpar directory is stored,
    checks whether it is empty, and reads the text inside it if it is not.
...
def tpar_set_dir(directory: str) -> None:
    """
    Ensures that the string provided is a valid path to a directory, then saves it as the
    directory in which to run all other TPAR related saving/loading commands.
    ...
    # show the user a list of the tpar files in this directory to check that they've got the
    # right one
    ...
    # If it isn't already set up as a git repo, do so. If it is, and it's set up to track a
    # remote, print a big warning.
    try:
        repo = git.Repo(directory)
        remote = repo.remote()
        print(
            f"WARNING: This directory is already in a repository at {repo.working_tree_dir}, set "
            f"up to track {remote.url}.\nYou are advised not to run save/load commands in this "
            f"repository, as it will interfere with other files stored here."
        )
    except ValueError:
        pass
...
def tpar_save_state(tag_name: None | str = None) -> None:
    ...
    Args: tag_name (optional): What to name the tag (also used as commit message). If None,
    just uses the ISO format of the current UTC time
    """
    if tag_name is None:
    ...
    if directory is None:
        print("No TPAR directory set, please use tpar_set_dir(<directory>) first to continue.")
        return
...
def tpar_show_saves() -> None:
    ...
    print(f"The following saved directory states are available:\n{'\n'.join(commit_messages)}")


def tpar_load_save(save_name: str) -> None:
    """
    Restores any modified files to the state they were in at the time of a specified tag. Does
    not alter files that have been created since that tag.
    ...
    try:
        ...
        print("No save/tag under that name, consider using tpar_show_saves and trying again.")
        return

    # Make sure that everything committed in advance of the rollback, just in case it needs to be
    # reverted. Just use current ISO datetime for this.
    time_now = datetime.datetime.now(datetime.timezone.utc)
...
def tpar_load_latest_save() -> None:
    """
    Restores any modified files to the state they were in at the time of the most recent tag.
    Does not alter files that have been created since that tag.
    """
    directory = tpar_check_dir()
    if directory is None:
        print("No TPAR directory set, please use tpar_set_dir(<directory>) first to continue.")
    ...
    # Search for the latest tag by just running a max() against the datetime objects attached to
    # their commits
